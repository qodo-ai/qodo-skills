...
test('captured Kiro HTML satisfies the core-only directory contract', () => {
  assert.deepEqual(verifyKiroDocument(fixture, context).map(({ id }) => id), ['qodo']);
});

test('accepts the core listing in raw JSON, JSON script data, and split Next.js data', () => {
  ...
  for (const document of [json, `<script type="application/json">${directory}</script>`,
    ...
    assert.deepEqual(verifyKiroDocument(document, context).map(({ id }) => id), ['qodo']);
  }
});
...
test('unrelated metadata cannot add conflicting cards to a valid directory', () => {
  ...
  for (const document of [JSON.stringify(directory),
    ...
    assert.equal(verifyKiroDocument(document, context).length, 1);
  }
});
...
test('JSON decoding preserves escaped quotes, backslashes, Unicode and braces inside strings', () => {
  ...
  assert.equal(verifyKiroDocument(flightDocument(records), context).length, 1);
  const escaped = JSON.stringify({ powers: records }).replaceAll('/', '\\u002F');
  assert.equal(verifyKiroDocument(escaped, context).length, 1);
...
test('requires each listing to carry the configured path, branch and repository in one record', () => {
  ...
  for (const [entry, message] of invalid) {
    assert.throws(() => verifyKiroDocument(flightDocument([entry]), context), message);
  }
  assert.throws(() => verifyKiroDocument(flightDocument([
    { name: 'qodo' }, { ...entries[0], name: 'unrelated' },
  ]), context), /expected path kiro-power/);
  assert.throws(() => verifyKiroDocument(flightDocument([]), context), /qodo.*missing/);
  assert.throws(() => verifyKiroDocument(flightDocument([
...
test('older immutable contracts retain their exact release-branch requirement', async (t) => {
  ...
  assert.equal((await verifyMarketplace('kiro', context, previous)).length, 1);
});
...
test('clone URLs must identify the same repository as the displayed tree', () => {
  ...
  assert.equal(verifyKiroDocument(flightDocument(https), context).length, 1);
...
test('Kiro release packet provides directory entries from the canonical marketplace contract', () => {
  ...
  try {
    ...
    assert.equal(verifyKiroDocument(JSON.stringify(desired), context).length, 1);
