...
- The resolver's **prior fix-summary comments** are the top-level PR/MR comments (Gerrit: change messages) whose body contains the verbatim marker `Generated by Qodo PR Resolver skill` **and** are authored by the account the resolver posts as. Only this skill writes that marker, so human `fix:` commits and Qodo's own review comments are invisible to detection.
...
**Dedicated oscillation section.** If any issue is tagged ⚠️ Contradiction (or 🔁 Repeat at a `file:line` that previously flipped direction), render a section **above** the main issue table grouping every such candidate, headed by the exact warning:
