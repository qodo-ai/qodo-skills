...
#### Step 1a: Detect prior resolver pass (oscillation-guard activation)

Before fetching the review, scan the branch history for the **resolver commit signature** to decide whether the oscillation guard (Step 3c) is active at all. Resolver fix commits carry a `Qodo-PR-Resolver-Round: <N>` trailer (added in Steps 6/7), which distinguishes them from ordinary human `fix:` commits:

```bash
git log --grep 'Qodo-PR-Resolver-Round:' origin/<base-branch>..HEAD
```

- If this returns **no commits**, no prior resolver pass exists → record `PRIOR_RESOLVER_PASS = false`. The oscillation guard is dormant: Step 3c will treat every issue as untagged.
- If it returns **one or more commits**, record `PRIOR_RESOLVER_PASS = true` and note the highest round number seen. Step 3c will build the decision ledger and tag issues.

This makes oscillation prevention trigger **only** on a detected prior resolver signature, never on unrelated human `fix:` commits.
...
Qodo re-reviews on every push, so the same code can be flagged across several rounds — sometimes with the *opposite* suggestion of a fix you already applied. Without memory of prior rounds, the resolver flip-flops and the PR never converges. Prevent this by reading your own trail before deciding anything.

Only run this step when Step 1a recorded `PRIOR_RESOLVER_PASS = true`. Build a **decision ledger keyed by `file:line`** from records you wrote in prior rounds (no new endpoints — reuse the Step 3 comment-fetch commands, plus git log):
- Your prior **fix-summary comments** (body contains `Generated by Qodo PR Resolver skill`) — fixed/deferred issues with reasons.
- Your prior **inline replies** (`✅ **Fixed** — …` / `⏭️ **Deferred** — …`) on each thread.
- Prior resolver fix commits (signature trailer): `git log --grep 'Qodo-PR-Resolver-Round:' origin/<base-branch>..HEAD`.

Then **tag each issue** parsed in Step 4 by looking it up in the ledger **by its `file:line` key** (fall back to title match only when line numbers have shifted):
- **🔁 Repeat** — the `file:line` matches a ledger entry already **fixed** in a prior round and is resurfacing.
- **⚠️ Contradiction** — looked up at the same `file:line`, the new suggestion would **reverse** a prior applied fix, or re-raises something the user **deliberately deferred**. This is the oscillation signal.
- **(untagged)** — not seen before; handle normally.

If the ledger is empty (first round) treat all issues as untagged. If history fetch fails, **say so** — do not silently behave as if there is no prior round.
...
For any issue tagged in Step 3c, prefix its **Action** cell with the tag (`🔁 Repeat — ` or `⚠️ Contradiction — `) so the user sees at a glance which findings are resurfacing or flip-flopping.

**Dedicated oscillation section.** If any issue is tagged ⚠️ Contradiction (or 🔁 Repeat at a `file:line` that previously flipped direction), render a distinct section **above** the main issue table that groups every such candidate, headed by the exact warning:

> ⚠️ Possible oscillation — Qodo is suggesting the opposite of a prior resolver action at this location.

List each candidate with its `file:line`, the prior decision + round number, and the new contradicting suggestion. These issues still also appear in the main table (with their tag prefix) for completeness.
...
- **⚠️ Contradiction** — do NOT auto-apply. Present the prior decision + rationale (with round number) alongside the new contradicting suggestion, and AskUserQuestion defaulting to **⏭️ "Defer"** (alternatives: ✅ "Apply" / 🔧 "Modify"). "Defer" holds the prior decision: reply to the thread explaining the prior decision was deliberate and resolve it — this is what breaks the oscillation.
...
- **🛑 Hard stop (3rd oscillation cycle)** — if a `file:line` has already flipped direction ≥2 times in the ledger, **refuse to apply** any further change there even if the user picks ✅ Apply, unless the user gives an **explicit override message** naming the location. Otherwise resolve the thread and route it to the Step 8 "Skipped to prevent oscillation" category. See [convergence.md](./resources/convergence.md#oscillation-cycle-counting-and-hard-stop).

- For each remaining issue marked as "Fix" (starting with CRITICAL) — **plus**, in single-finding mode, the lone issue even if marked "Defer":
  ...
  - **If "Apply fix" / "Apply fix anyway" selected:**
    ...
    - **GitHub / GitLab / Bitbucket / Azure DevOps:** Git commit the fix with the resolver signature trailer (round N = highest prior round from Step 1a + 1, or 1 on the first pass): `git add <modified-files> && git commit -m "fix: <issue title>" -m "Qodo-PR-Resolver-Round: <N>"`
...
**Oscillation guard:** issues tagged **🔁 Repeat** or **⚠️ Contradiction** in Step 3c are **excluded from automatic application** — auto-fix is exactly where unattended flip-flop churn happens. Handle them **by tag type** (do not lump together): **⚠️ Contradiction** → present the Contradiction options (Defer / Apply / Modify), defaulting to **Defer** if the user wants zero prompts; **🔁 Repeat** → re-read the current code and **Defer** if the prior fix is intact, else treat as a normal fix; **🛑 hard stop** (≥2 flips at a `file:line`) → refuse and route to the "Skipped to prevent oscillation" category. Report all held/excluded tagged issues in the Step 8 summary — never silently skip them. See [convergence.md](./resources/convergence.md).
