...
- Prior fix commits: `git log --oneline --grep '^fix:' origin/<base-branch>..HEAD`.

Then **tag each issue** parsed in Step 4 against the ledger:
- **🔁 Repeat** — same title/location was already **fixed** in a prior round and is resurfacing.
- **⚠️ Contradiction** — the new suggestion would **reverse** a prior applied fix, or re-raises something the user **deliberately deferred**. This is the oscillation signal.
...
- **⚠️ Contradiction** — do NOT auto-apply. Present the prior decision + rationale (with round number) alongside the new contradicting suggestion, and AskUserQuestion defaulting to **🛑 "Hold prior decision"** (alternatives: 🔄 "Apply the reversal anyway" / 🔧 "Modify"). On "Hold", reply to the thread explaining the prior decision was deliberate and resolve it — this is what breaks the oscillation.
...
- **Held prior decision** (⚠️ Contradiction kept as-is): set `"unresolved": false`, and make the reply state the prior decision was deliberate (see [convergence.md](./resources/convergence.md)) so the rationale is recorded on the thread for the next round. List these under the Deferred section of the summary with the oscillation reason.
