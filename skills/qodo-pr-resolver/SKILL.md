...
**Also scan for prior resolver activity (oscillation guard enablement):**
- Look for the resolver-authored signature commit message (T1 unified commit format), e.g. `fix: resolve Qodo review findings`:
  - `git log --oneline origin/<base-branch>..HEAD --grep '^fix: resolve Qodo review findings$'`
- If found, treat this PR as having prior resolver rounds and ensure Step 3c history loading is performed (do not assume first-round behavior).
...
Build a **decision ledger** from records you wrote in prior rounds (no new endpoints — reuse the Step 3 comment-fetch commands, plus git log):
- Your prior **fix-summary comments** (body contains `Generated by Qodo PR Resolver skill`) — fixed/deferred issues with reasons.
- Your prior **inline replies** (`✅ **Fixed** — …` / `⏭️ **Deferred** — …`) on each thread (accept both bolded and non-bolded variants when matching).
- Prior fix commits (including the resolver signature commit): `git log --oneline --grep '^fix:' origin/<base-branch>..HEAD`.

Then **tag each issue** parsed in Step 4 against the ledger by doing a primary lookup at the same **`file:line`** (and only then using title as a secondary hint):
- **🔁 Repeat** — same `file:line` was already **fixed** in a prior round and is resurfacing.
- **⚠️ Contradiction** — at the same `file:line`, the new suggestion would **reverse** a prior applied fix, or re-raises something the user **deliberately deferred**. This is the oscillation signal.
...
If **any** issues are tagged in Step 3c (🔁 Repeat / ⚠️ Contradiction), you MUST surface them in a dedicated subsection before the main table:

#### ⚠️ Possible oscillation

⚠️ Possible oscillation — Qodo is suggesting the opposite of a prior resolver action at this location.

List all tagged issues here (in Qodo’s original order) so they are visually grouped, then continue with the full issues table.

Also, for any issue tagged in Step 3c, prefix its **Action** cell with the tag (`🔁 Repeat — ` or `⚠️ Contradiction — `) so the user sees at a glance which findings are resurfacing or flip-flopping.
...
- **⚠️ Contradiction** — do NOT auto-apply. Present the prior decision + rationale (with round number) alongside the new contradicting suggestion, and AskUserQuestion defaulting to **⏭️ Defer** with options exactly: **Defer / Apply / Modify**. If this location has hit the hard stop (≥2 prior flips at the same `file:line`), refuse auto-application unless the user provides the explicit override message from [convergence.md](./resources/convergence.md).
...
**Oscillation guard:** issues tagged **🔁 Repeat** or **⚠️ Contradiction** in Step 3c are **excluded from automatic application** — auto-fix is exactly where unattended flip-flop churn happens. Collect them and present a **single** AskUserQuestion (the Contradiction options from Step 6); if the user wants zero prompts, default each to **🛑 Hold prior decision** and reply with the rationale. Report all held/excluded tagged issues in the Step 8 summary — never silently skip them. See [convergence.md](./resources/convergence.md).
...
- **Skipped to prevent oscillation** (🔁 Repeat / ⚠️ Contradiction not auto-applied): set `"unresolved": false`, and make the reply state the prior decision was deliberate / previously addressed (see [convergence.md](./resources/convergence.md)) so the rationale is recorded on the thread for the next round. List these under the **"Skipped to prevent oscillation — recommend human resolution."** section of the Step 8 summary (not under Deferred).
