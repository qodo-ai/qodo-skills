...
Before deciding whether the oscillation guard (Step 3c) is active at all, detect a **prior resolver pass**. The detection signal differs by provider:

**GitHub / GitLab / Bitbucket / Azure DevOps — commit signature.** Resolver fix commits carry a `Qodo-PR-Resolver-Round: <N>` trailer (added in Steps 6/7), which distinguishes them from ordinary human `fix:` commits. The trailer is unique to resolver commits, so scan the current branch's reachable history — no base-branch range is needed (and the base branch isn't known until Step 2):

```bash
git log --grep 'Qodo-PR-Resolver-Round:' HEAD
```

**Gerrit — comment trail.** Gerrit keeps all fixes in a single amended commit (Step 6), so the round trailer is never written to git there. Detect a prior pass instead from the resolver's own durable trail on the change: a prior **fix-summary comment** (body contains `Generated by Qodo PR Resolver skill`) or a prior `✅ **Fixed**` / `⏭️ **Deferred**` inline reply. This signal is only available once comments are fetched, so on Gerrit finalize `PRIOR_RESOLVER_PASS` during Step 3 (the round number is the count of prior resolver summary comments, not a git value).

- If **no** prior-pass signal is found, record `PRIOR_RESOLVER_PASS = false`. The oscillation guard is dormant: Step 3c will treat every issue as untagged.
- If a prior-pass signal **is** found, record `PRIOR_RESOLVER_PASS = true` and note the highest round number seen (trailer round on GitHub/GitLab/Bitbucket/Azure DevOps; prior-summary count on Gerrit). Step 3c will build the decision ledger and tag issues.

This makes oscillation prevention trigger **only** on a detected prior resolver pass, never on unrelated human `fix:` commits.
...
Only run this step when Step 1a recorded `PRIOR_RESOLVER_PASS = true`. Build a **decision ledger keyed by `file` + issue `title`** — a stable identity that survives a prior fix shifting the line numbers. (The raised `line` is recorded too, but only to disambiguate multiple same-title findings in one file — never as the primary match key.) Build it from records you wrote in prior rounds (no new endpoints — reuse the Step 3 comment-fetch commands, plus git log):
...
- Prior resolver fix commits (signature trailer): `git log --grep 'Qodo-PR-Resolver-Round:' HEAD` (GitHub/GitLab/Bitbucket/Azure DevOps). On **Gerrit** the trailer is never written (single amended commit), so rely on the comment-trail records above.
...
- **🛑 Hard stop (3rd oscillation cycle)** — if a ledger entry (its `file` + `title` key) has already flipped direction ≥2 times, **refuse to apply** any further change at that location even if the user picks ✅ Apply, unless the user gives an **explicit override message** naming the location. Otherwise resolve the thread and route it to the Step 8 "Skipped to prevent oscillation" category. See [convergence.md](./resources/convergence.md#oscillation-cycle-counting-and-hard-stop).
