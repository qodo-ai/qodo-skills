...
#### Step 3c: Compute the review-round diff scope

Qodo re-reviews on every push, so a review can surface issues on code that did **not** change in the current round — noise the reviewer didn't touch this time (reported during the Marvell PoV). Compute the diff scope **once here**; Step 4 uses it to separate current-round issues from carried-over ones.

1. **PR base:** `PR_BASE = git merge-base origin/<baseRef> HEAD` (`baseRef` from the PR/MR — e.g. `gh pr view <n> --json baseRefName`).
2. **Round base** — the start of the current round:
   - If a prior **resolver fix-summary** comment exists on this PR (a top-level comment containing the verbatim marker `Generated by Qodo PR Resolver skill`), set `ROUND_BASE` = the newest commit whose commit date ≤ that comment's timestamp (commits from `gh pr view <n> --json commits`). This means "everything since we last resolved".
...
#### Classify issues by scope (from Step 3c)
...
**When not `SINGLE_ROUND`, group the display by scope:**
1. **Current round** — the main table above. Only these enter the resolution loop by default.
2. **➖ Carried over from earlier review rounds — defer or revisit** — `scope: carried_over` issues, listed compactly (title · location). Not walked through by default.
3. **➖ Outside the PR diff — likely review error, defer** — `scope: outside_pr` issues.

Then ask once with AskUserQuestion — **"Include N carried-over issue(s) in this round?"**, default **No**. If Yes, carried-over issues join the current-round working set; `outside_pr` issues are never auto-included. Any carried-over / outside-PR issue not processed is reported in the Step 8 summary under **"Deferred — outside current review round"**, never silently dropped.

### Step 5: Ask user for fix preference

**Working set:** After Step 4's scope grouping, "the issues" below means the **current-round working set** — current-round issues plus any carried-over issues the user opted to include. (When `SINGLE_ROUND`, that is simply all issues.)
