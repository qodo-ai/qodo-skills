...
Gerrit keeps all fixes in a single amended commit, so the `Qodo-PR-Resolver-Round:` commit trailer that SKILL.md Step 1a relies on is **never written on Gerrit**. Detect a prior resolver pass (to set `PRIOR_RESOLVER_PASS`) from the durable comment trail instead: a prior **change message** containing `Generated by Qodo PR Resolver skill`, or a prior `✅ **Fixed**` / `⏭️ **Deferred**` inline reply (fetched above). The round number is the count of prior resolver summary messages. With this signal present, Step 3c builds the decision ledger normally.
