...
Round detection reads only the resolver's own comments — the **same comment-trail algorithm Step 3c runs for every provider**; git is never consulted (no provider writes tool metadata into commits). Detect a prior pass from a prior **change message** containing the marker `Generated by Qodo PR Resolver skill`, authored by the account the resolver posts as. The round number comes from the `## Qodo Fix Summary — Round N` heading (`max` of the parsed N and the marker-bearing summary-message count). **Note:** Gerrit prepends a `Patch Set N:` line to change messages, so match the heading **anywhere** in the body, not only the first line.
