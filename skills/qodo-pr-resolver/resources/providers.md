...
- ⏭️ **Issue Title** (Severity) - Deferred - brief reason
- 🆗 **Issue Title** (Severity) - False positive - brief reason
- 🎯 **Issue Title** (Severity) - Intentional - brief reason
- 🚫 **Issue Title** (Severity) - Won't fix - brief reason
...
Emoji per closed reason: ⏭️ `deferred` · 🆗 `false_positive` · 🎯 `intentional` · 🚫 `rejected` (won't fix). List only the rows that apply.
