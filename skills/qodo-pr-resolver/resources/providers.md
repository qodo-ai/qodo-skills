...
**Summary format:** (the `— Round N` heading and the `Generated by Qodo PR Resolver skill` footer are how the *next* resolver run detects this round — see SKILL.md Step 3c; `N` is the current round number)
